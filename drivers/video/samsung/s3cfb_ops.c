...
//Apollo +
	{
	  void *vsrc_base;
		...
		vsrc_base = phys_to_virt((unsigned long)(0x50000000));
		pci_head = (HEADCOLOR *)vsrc_base;
		printk("%d %d %d %d\n", pci_head->w, pci_head->h, var->xres, var->yres);
		if((pci_head->w == var->xres) && (pci_head->h == var->yres))
		{
			memcpy(fb->screen_base, vsrc_base+sizeof(HEADCOLOR), fix->line_length * var->yres);
			return 0;
		}
	}
//Apollo -
