...
static void report_abs(void)
{
  ...
  else
  {
  	x=0;
  	y=0;
  	if(key_state0 & 0x01) //up
  	  x=-512;
  
  	if(key_state0 & 0x02) //down
  	  x=512;
  	
  	if(key_state0 & 0x04) //left
  	  y=512;
  
  	if(key_state0 & 0x08) //right
  	  y=-512;
  ...
  //旋转
  if(gsensor_st == 1) //旋转90度
  {
    tmp = y_r;
    y_r = -x_r;
    x_r = -tmp;
  }
  if(gsensor_st == 2) //旋转180度
  {
    x_r = -x_r;
    y_r = -y_r;
  }
  if(gsensor_st == 3) //旋转270度
  {
    tmp = x_r;
    x_r = y_r;
    y_r = tmp;
  ...
  input_report_abs(mma8452_idev->input, ABS_X, x_r);
  input_report_abs(mma8452_idev->input, ABS_Y, y_r);
  input_report_abs(mma8452_idev->input, ABS_Z, z_r);
				   ...
				   const struct i2c_device_id *id)
{
  ...
  adapter = to_i2c_adapter(client->dev.parent);
	...
	if(dev_ver != 1)
	{
  	...
  	result = i2c_smbus_read_byte_data(client, MMA8452_WHO_AM_I)&0xFF;
...
#endif
 device_create_file(switch_dev, &dev_attr_gsensor_st);
 device_create_file(switch_dev, &dev_attr_gsensor_disable);
	return result;
