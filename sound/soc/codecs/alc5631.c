...
tinymix:
¶ú»ú
21      ENUM    1       HPR Mux                                  Right DAC
22      ENUM    1       HPL Mux                                  Left DAC
À®°È
 ...
 */
int rt5631_write_index(struct snd_soc_codec *codec,
...
EXPORT_SYMBOL_GPL(rt5631_write_index);
/**
 * rt5631_read_index - read index register of 2nd layer
 */
unsigned int rt5631_read_index(struct snd_soc_codec *codec,
				unsigned int reg)
{
//	unsigned int value;
//
...
EXPORT_SYMBOL_GPL(rt5631_read_index);
...
static const struct snd_soc_dapm_widget rt5631_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("I2S", RT5631_PWR_MANAG_ADD1,          ///fighter
...
static int rt5631_reg_init(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < RT5631_INIT_REG_LEN; i++){
		rt5631_write_index(codec, init_list[i].reg, init_list[i].val);
}
	return 0;
}
static int rt5631_reg_wr(struct snd_soc_codec *codec)
{
	int tmp = 0;

	/************************RT5631_PWR_MANAG_ADD1****************************/		
	tmp = 1<<RT5631_PWR_MAIN_I2S_BIT | 1<<RT5631_PWR_CLASS_D_BIT | 1<<RT5631_PWR_DAC_L_CLK_BIT | 1<< RT5631_PWR_DAC_R_CLK_BIT | 1<<RT5631_PWR_DAC_REF_BIT \
	      | RT5631_PWR_DAC_L_TO_MIXER | RT5631_PWR_DAC_R_TO_MIXER;
printk("tmp = 0x%04x ***********************\n",tmp);	      
	rt5631_write_index(codec,RT5631_PWR_MANAG_ADD1,tmp);
	
	/************************RT5631_PWR_MANAG_ADD2****************************/
	tmp = RT5631_PWR_SPKMIXER_L | RT5631_PWR_SPKMIXER_R | RT5631_PWR_PLL1;
printk("tmp = 0x%04x ***********************\n",tmp);
	rt5631_write_index(codec,RT5631_PWR_MANAG_ADD2,tmp);
	
	/************************RT5631_PWR_MANAG_ADD3****************************/
	tmp = RT5631_PWR_VREF ;
printk("tmp = 0x%04x ***********************\n",tmp);	
	rt5631_write_index(codec,RT5631_PWR_MANAG_ADD3,tmp);
	/************************RT5631_PWR_MANAG_ADD4****************************/
	tmp = RT5631_PWR_SPK_L_VOL | RT5631_PWR_SPK_R_VOL |RT5631_PWR_LOUT_VOL |RT5631_PWR_ROUT_VOL;
printk("tmp = 0x%04x ***********************\n",tmp);	
	rt5631_write_index(codec,RT5631_PWR_MANAG_ADD4,tmp);
	
	tmp = rt5631_read_index(codec,RT5631_PWR_MANAG_ADD1);
	printk("tmp = 0x%04x ++++++++++++++++++++\n",tmp);
	tmp = rt5631_read_index(codec,RT5631_PWR_MANAG_ADD2);
	printk("tmp = 0x%04x ++++++++++++++++++++\n",tmp);
	tmp = rt5631_read_index(codec,RT5631_PWR_MANAG_ADD3);
	printk("tmp = 0x%04x ++++++++++++++++++++\n",tmp);	
	tmp = rt5631_read_index(codec,RT5631_PWR_MANAG_ADD4);
	printk("tmp = 0x%04x ++++++++++++++++++++\n",tmp);	
...
//#define RT5631_FORMAT	(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | \
//	SNDRV_PCM_FMTBIT_S24_LE)
//fighter-
